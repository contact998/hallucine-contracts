...
describe("dist/ versionné", () => {
  it("correspond exactement à ce que src/ produit", () => {
    const temoin = mkdtempSync(join(tmpdir(), "contracts-dist-"));
    try {
      execFileSync(
        "npx",
      ...
      /* Les fichiers cachés ne sont pas des sorties de compilation : macOS pose
         un `.metadata_never_index` dans dist/, et le test tombait sur lui plutôt
         que sur un vrai décalage (même filtre que @hallucine/gonflable). */
